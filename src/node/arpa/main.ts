...
let log = tsa.common.logger.get_logger({id:colors.green("arpa_main")}) ;
...
let config = tsa.node.io.read_json("arpa_config.json")
...
async function handle_binanceus_config_entry(entry : any){
    ...
    let buf = await tsa.node.cryptography.aes_192_decrypt_file(runtime_password as string, keyfile)
    let keys = JSON.parse(buf.toString('utf8'))

    let balancer = new tsa.node.trading.binanceus_balancer.BinanceUsBalancer({...parameters, keys})
...
interface Uni_v2_entry {
    ...
    balance_parameters : tsa.common.trading.portfolio_balancer_lib.BalanceParams ,
    evm_parameters  :  tsa.common.web3.evm_balancers.BaseAMMParams , 
} 
...
async function handle_uniswap_v2_config_entry(entry : Uni_v2_entry ) {
    ...
    const {smart_wallet} = tsa.common.web3 ;
    ...
    let uni_ops  = R.mergeAll( [balance_parameters, evm_parameters , {smartWallet : sw} ] ) as tsa.common.web3.evm_balancers.AMMParams  ;

    log(`Initializing UNI_V2 Balancer (${balance_parameters.logger_id})`) ; 
    var w = new tsa.common.web3.evm_balancers.UniV2Balancer( uni_ops ) ; await w.init()  ;
